...
const NetworkTab = ({ network, toggleNetwork, isSelected }: { network: string, toggleNetwork: (network: string) => void, isSelected: boolean }) => {
    return (
        <div className={`flex items-center justify-start gap-2 cursor-pointer hover:bg-background/15 bg-background/10 px-2 py-2 rounded-sm ${isSelected ? 'bg-secondary/20 ring ring-secondary/40' : ''}`} onClick={() => toggleNetwork(network)}>
