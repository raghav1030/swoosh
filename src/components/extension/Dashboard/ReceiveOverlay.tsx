...
const ReceiveOverlay = ({ isOpen, close, wallet }: ReceiveOverlayProps) => {
    ...
    return (
        <div className={`absolute inset-0 w-full h-full z-[100] flex flex-col justify-end transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
            {/* Backdrop */}
            <div
                className="flex-1 w-full cursor-pointer bg-black/40 backdrop-blur-sm transition-opacity duration-500"
                style={{ opacity: isOpen ? 1 : 0 }}
                onClick={close}
            />

            {/* Sheet Content */}
            <div className="w-full bg-black/95 backdrop-blur-xl border-t border-secondary/10 rounded-t-3xl flex flex-col items-center shadow-2xl relative pb-8">
